...
export default function HomeSection(data) {
  function definePetSizes() {
    let sizesString = "";
    if (data.homeData.hosting.cat || data.homeData.hosting.dog.small.isHosted) sizesString += ",Chico";
    ...
    <div className="bg-[#2B2E4A] sm:rounded-2xl p-[16px] sm:p-[20px] md:p-[24px] lg:p-[28px] text-[#2B2E4A]">
      ...
      {!data.homeData && data.idMatch && (
        <div className="flex flex-col-reverse lg:flex-row w-full gap-[28px] lg:gap-[48px] items-center">
          <NewHouseCard />
          <p className="text-white font-light text-[16px] md:text-[20px] text-center w-full lg:w-1/2">Para poder recibir solicitudes de reserva debes agregar tu alojamiento</p>
        </div>
      )}
      {data.homeData && (
        <>
          ...
          <div className="flex flex-col sm:flex-row gap-[12px] text-[20px] p-[12px] md:p-[20px] bg-[#F2F2F2] rounded-[10px] font-bold justify-between">
            <div className="flex flex-col gap-[12px] sm:gap-7 basis-1/3">
              <p className="h-fit sm:h-[55px] text-[16px] md:text-[18px] lg:text-[20px]">
                ...
                <span className="font-normal text-[14px] md:text-[16px] lg:text-[18px]">
                  {data.homeData.hosting.cat.isHosted && (data.homeData.hosting.dog.small.isHosted || data.homeData.hosting.dog.medium.isHosted || data.homeData.hosting.dog.big.isHosted)
                    ? "Perros y Gatos"
                    : data.homeData.hosting.cat.isHosted
                    ? "Gatos"
                    : data.homeData.hosting.dog.small.isHosted || data.homeData.hosting.dog.medium.isHosted || data.homeData.hosting.dog.big.isHosted
                    ? "Perros"
                    : null}
                </span>
              </p>
            ...
            <div className="flex flex-col gap-[12px] sm:gap-7 basis-1/3">
              <p className="h-fit sm:h-[55px] text-[16px] md:text-[18px] lg:text-[20px]">
                Tamaño: <span className="font-normal text-[14px] md:text-[16px] lg:text-[18px]">{definePetSizes()}</span>
              </p>
              <div>
            ...
            <div className="flex flex-col gap-[12px] sm:gap-7 basis-1/3 font">
              <p className="h-fit sm:h-[55px] text-[16px] md:text-[18px] lg:text-[20px]">
                Tiene Mascota: <span className="font-normal text-[14px] md:text-[16px] lg:text-[18px]">{data.homeData.hasPet ? "Sí" : "No"}</span>
              </p>
              <div>
                <p className="text-[16px] md:text-[18px] lg:text-[20px]">Descripción:</p>
                <p className="text-[14px] md:text-[16px] lg:text-[18px] font-normal">{data.homeData.description}</p>
              </div>
            </div>
          </div>
        </>
