...
export default function Search() {
  ...
  const [
    { state, city, pettype, petsize, initialdate, enddate },
    setSearchQuery,
  ] = useState({});
  ...
  useEffect(() => {
    const { state, city, pettype, petsize, initialdate, enddate } =
      router.query;
    console.log(router.query);
    setSearchQuery({ state, city, pettype, petsize, initialdate, enddate });
    const BASE_URL = process.env.NEXT_PUBLIC_BASE_URL;
    fetch(
      `${BASE_URL}/accommodation?state=${state}&city=${city}&pettype=${pettype}&petsize=${petsize}`
    )
  ...
  return (
    <main className="flex flex-col lg:flex-row min-h-screen bg-white">
      ...
      <section className="flex flex-col py-[12px] px-[16px] w-full lg:w-[62%] lg:pr-[20px] lg:ps-[10px] xl:py-[40px] xl:pr-[40px] xl:ps-[40px] lg:mt-[90px] gap-[20px] lg:gap-[32px] items-center h-full">
        <h1 className="text-[#2B2E4A] font-semibold font-[Raleway] text-center text-[20px] lg:text-[28px] ">
        ...
        {accommodationList && accommodationList.length > 0 ? (
          ...
          <p className="">
            Lo sentimos, no encontramos ningun anfitiron que coincidiera con tus
            necesidades
          </p>
        )}
      </section>
